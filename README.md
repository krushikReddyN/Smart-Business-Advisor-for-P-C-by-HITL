...
A Streamlit-in-Snowflake application for Property & Casualty insurance underwriting, powered by Snowflake Cortex AI, ML Classification, and Geospatial functions.
...
```
HITL_SBA_DB.PC_INSURANCE
├── PROPERTY_MASTER        (10,000 rows)  — Core property data + policy info
├── MODEL_OUTPUT           (10,000 rows)  — Risk scores, categories, premiums
├── RISK_GEOSPATIAL        (10,000 rows)  — Flood, wildfire, crime, coast distance
├── WEATHER_HISTORY        (10,000 rows)  — Rainfall, storms, hurricanes, temperature
├── AERIAL_ANALYSIS        (10,000 rows)  — Roof condition, material, structural risk
├── PREMIUM_BENCHMARK      (10,000 rows)  — Market premium ranges
├── CLAIMS_HISTORY         (~30,000 rows) — Historical claims with type & severity
├── IMAGE_ANALYSIS_STAGE   (internal)     — Image uploads for AI analysis
└── IMAGE_STAGE            (internal)     — AI Assistant image uploads
```

All tables join on `PROPERTY_ID`. Data spans 30 US cities across 20 states.

## Application Tabs

| Tab | Description |
|-----|-------------|
| **Portfolio** | Risk donut, write/decline by state, premium scatter, risk histogram, construction analysis, coastal exposure |
| **Property Lookup** | Per-property underwriting decision, risk factors (geo/weather/aerial/benchmark), claims history |
| **Geospatial** | Risk map, flood/wildfire distributions, H3 hexagonal aggregation with heatmap, proximity search |
| **Claims** | Claims KPIs, yearly trends, severity breakdown, claim type analysis |
| **ML Classifier** | Train `SNOWFLAKE.ML.CLASSIFICATION` on selected features, predict risk for all properties, ad-hoc prediction |
| **Image Analysis** | Upload property/aerial images for Cortex AI analysis (pixtral-large), portfolio data assessment (llama3.1-70b) |
| **AI Assistant** | Chat interface with full portfolio context via `SNOWFLAKE.CORTEX.COMPLETE`, optional SQL generation, image analysis |
| **Data Explorer** | Sortable/filterable dataframe with progress-bar columns for risk, hazard, and roof scores |

## Snowflake Features Used

- **Cortex AI** — `SNOWFLAKE.CORTEX.COMPLETE` with pixtral-large (vision), claude-3-5-sonnet (vision), llama3.1-70b (text)
- **ML Classification** — `SNOWFLAKE.ML.CLASSIFICATION` for risk category prediction
- **Geospatial** — `H3_LATLNG_TO_CELL`, `ST_DISTANCE`, `ST_MAKEPOINT`
- **Streamlit in Snowflake** — Full app with sidebar filters, option_menu nav, pydeck maps, Altair charts
- **Snowpark** — `session.sql()`, `session.file.put_stream()`, `@st.cache_data`

## Prerequisites

- Snowflake account with `ACCOUNTADMIN` role (or equivalent privileges)
- Warehouse (e.g., `COMPUTE_WH`)
- Cortex AI models enabled in your region (`pixtral-large`, `llama3.1-70b`, `claude-3-5-sonnet`)
- Streamlit in Snowflake enabled

## Setup

### Step 1: Run the Data Setup SQL

Execute the entire SQL block below in a Snowflake worksheet. It creates the database, schema, all 7 tables, and 2 internal stages in one run.
...
3. Set database to `HITL_SBA_DB`, schema to `PC_INSURANCE`
4. Paste the full application code (see `Setup` file in workspace)
5. Add `streamlit-option-menu` to the packages list
6. Click **Run**
...
|--------|------|-------------|
| PROPERTY_ID | VARCHAR | Primary key (PROP-00001 to PROP-10000) |
| POLICY_NUMBER | VARCHAR | Policy identifier (POL-XXXXXX-ST) |
| PROVIDER_NAME | VARCHAR | Insurance provider (10 carriers) |
| ADDRESS | VARCHAR | Street address |
| CITY | VARCHAR | One of 30 US cities |
| STATE | VARCHAR | Two-letter state code |
| ZIP | VARCHAR | ZIP code |
| LATITUDE | FLOAT | GPS latitude |
| LONGITUDE | FLOAT | GPS longitude |
| PROPERTY_TYPE | VARCHAR | Residential (70%) or Commercial (30%) |
| YEAR_BUILT | NUMBER | 1940–2024 |
| ROOF_AGE | NUMBER | Years (capped at building age) |
| CONSTRUCTION_TYPE | VARCHAR | Frame, Masonry, Steel, Concrete, Mixed |
| INSURED_VALUE | NUMBER | $100K–$15M |

### MODEL_OUTPUT
| Column | Type | Description |
|--------|------|-------------|
| PROPERTY_ID | VARCHAR | Foreign key |
| RISK_SCORE | FLOAT | 0.0–1.0 composite risk score |
| RISK_CATEGORY | VARCHAR | High (20%), Medium (50%), Low (30%) |
| RECOMMENDED_ACTION | VARCHAR | Write or Decline |
| SUGGESTED_PREMIUM | FLOAT | Model-recommended premium |

### RISK_GEOSPATIAL
| Column | Type | Description |
|--------|------|-------------|
| PROPERTY_ID | VARCHAR | Foreign key |
| FLOOD_ZONE | VARCHAR | High, Medium, Low |
| WILDFIRE_RISK | VARCHAR | High, Medium, Low |
| CRIME_INDEX | NUMBER | 0–100 |
| DISTANCE_TO_COAST_KM | FLOAT | Distance to nearest coast |
| HAZARD_SCORE | FLOAT | 0.0–1.0 composite hazard |

### WEATHER_HISTORY
| Column | Type | Description |
|--------|------|-------------|
| PROPERTY_ID | VARCHAR | Foreign key |
| AVG_ANNUAL_RAINFALL | FLOAT | Inches per year |
| STORM_FREQUENCY | NUMBER | Storms per year |
| AVG_TEMPERATURE | FLOAT | Degrees Fahrenheit |
| HURRICANE_EVENTS_LAST_10Y | NUMBER | Hurricane count |

### AERIAL_ANALYSIS
| Column | Type | Description |
|--------|------|-------------|
| PROPERTY_ID | VARCHAR | Foreign key |
| ROOF_CONDITION_SCORE | FLOAT | 0.0–1.0 (1 = excellent) |
| ROOF_MATERIAL | VARCHAR | Shingle, Metal, Tile, etc. |
| STRUCTURAL_RISK_SCORE | FLOAT | 0.0–1.0 (1 = highest risk) |
| VEGETATION_DENSITY | VARCHAR | Low, Medium, High |

### PREMIUM_BENCHMARK
| Column | Type | Description |
|--------|------|-------------|
| PROPERTY_ID | VARCHAR | Foreign key |
| AVG_MARKET_PREMIUM | FLOAT | Market average premium |
| MIN_PREMIUM | FLOAT | Market minimum |
| MAX_PREMIUM | FLOAT | Market maximum |

### CLAIMS_HISTORY
| Column | Type | Description |
|--------|------|-------------|
| CLAIM_ID | VARCHAR | Primary key (CLM-000001+) |
| PROPERTY_ID | VARCHAR | Foreign key |
| CLAIM_YEAR | NUMBER | 2010–2025 |
| CLAIM_AMOUNT | NUMBER | $500–$500,000 |
| CLAIM_TYPE | VARCHAR | Flood, Storm, Fire, Theft |
| CLAIM_SEVERITY | VARCHAR | Low, Medium, High |

## Risk Score Formula

```
risk_score = 0.20 * hazard_score
           + 0.15 * structural_risk_score
           + 0.12 * (1 - roof_condition_score)
           + 0.10 * min(storm_frequency / 15, 1)
           + 0.08 * min(hurricane_events / 5, 1)
           + 0.15 * min(total_claims / 10, 1)
           + 0.10 * min(avg_claim_amount / 200000, 1)
           + 0.05 * crime_index / 100
           + 0.05 * noise(+-0.1)
```

**Risk category:** NTILE(10) on risk_score — deciles 1–3 = Low, 4–8 = Medium, 9–10 = High

**Recommended action:** High = 70% Decline, Medium = 10% Decline, Low = always Write

**Suggested premium:** `insured_value * 0.5% * (1 + risk_score * 3) * category_multiplier`

## Data Summary

| Table | Rows | Key Metrics |
|-------|------|-------------|
| PROPERTY_MASTER | 10,000 | 30 cities, 20 states, 10 providers |
| MODEL_OUTPUT | 10,000 | ~20% High, ~50% Medium, ~30% Low |
| RISK_GEOSPATIAL | 10,000 | 5 region types (coastal/wildfire/tornado/arid/inland) |
| WEATHER_HISTORY | 10,000 | Region-correlated weather patterns |
| AERIAL_ANALYSIS | 10,000 | Age-correlated roof/structural scores |
| PREMIUM_BENCHMARK | 10,000 | Hazard-adjusted market premiums |
| CLAIMS_HISTORY | ~30,000 | 30% zero-claim, tiered distribution |
